...
def plot_routes(coords, routes, title, out_path):
    ...
    if depot_xy[0] is not None:
        plt.scatter([depot_xy[0]], [depot_xy[1]], c="red", s=140, marker="s", label="Dépôt (0)")
    # Clients
    cx, cy = [], []
    for cid, (x, y) in coords.items():
        ...
        cy.append(y)
    if cx:
        plt.scatter(cx, cy, c="lightblue", s=40, label="Clients")
...
if __name__ == "__main__":
    ...
    # Load coordinates once (may be reloaded later if needed)
    ...
    if RUN_MGA:
        ...
        mga_routes = parse_routes_from_csv(mga_csv)
    ...
    if RUN_EXACT:
        ...
        coords = load_coords_from_json(instance_json)
        exact_routes = parse_routes_from_csv(exact_csv)
